...
*Геттеры :*
- `selectedCard(): string` - для получения id выбранной карточки.

*Методы :*
- `getCard(id: string): ICard` - возвращает карточку по `id`
- `getGalleryCards(): TGalleryCard[]` - возвращает массив карточек для отображения на главной странице.
...
*Поля :*
- `cards: ICard[]` - карточки в корзине.
- `events: IEvents` - экземпляр класса `EventEmitter` для инициации событий при изменении данных.

*Методы :*
- `add(card: ICard): void` - добавляет один карточка в начало массива и вызывает событие изменения массива.
- `remove(id: string): void` - удаляет из массива выбранный карточка по `id` и вызывает событие изменения массива и вызывает событие изменения массива.
- `getBasketViewCards(): TBasketCard[]` - получение карточки для вывода в модальном окне корзины.
- `getCost(): number` - для получения стоимости всей корзины.
- `getQuantity(): number` - для получения количества товаров в корзине.
- `clear(): void` - для очистки корзины.
- `getIdList(): string[]` - для получения списка id всех товаров в корзине.
- `contains(id: string): boolean` - для проверки нахождения выбранного товара в корзине по его id.
- `basketDataChanged(): void`  - для генерации события `basket-data:change` при изменении состояния корзины.
...
*Конструктор :*
- `constructor(protected container: HTMLElement, protected events: IEvents, action?: ICArdAction)` - принимает элемент карточки на основе шаблона, экземпляр класса `EventEmitter` и обработчик для реализации различных сценариев кнопки карточки.
...
- `canBuy(value: boolean)` - в зависимости от параметра меняет название кнопки карточки на `Удалить`
...
- `cards-list:changed` - при изменении общего списка карточек
- `card-preview:changed` - выбор карточки для просмотра в модальном окне
- `basket:remove` - при удалении карточки из корзины
- `basket:add` - при добавлении карточки в корзину
- `basket-data:change` - при изменении состояния корзины 
- `basket: open` - при открытии корзины
- `basket:submit` - для открытия формы ордера
- `order-payment:select` - выбор способа оплаты заказа
- `order-form:input` - изменение данных в форме способа оплаты
- `order-form:submit` - для открытия формы контактов 
- `order-data:change` - при изменении данных в ордере
- `contacts-form:input` - при изменении полей ввода формы контактов
- `contacts-form:submit` - при отправке готового ордера на сервер
- `contacts-data:change` - при изменении данных в контактах
- `order-success:submit` - генерируемое при нажатии "За новыми покупками"


### Презентер

Отвечает за взаимодействие компонентов Взаимодействие происходит за счет брокера событий, который их отслеживает. Роль презентера выполняет `index.ts`.